...
class MessageThreadEntry extends ThreadEntry {
    ...
    static function add($vars, &$errors=array()) {
        ...
        // A ticket message may consist of text, an uploaded voice message,
        // or both. This is the final validation point used by new tickets
        // and client-posted replies.
        elseif (!$vars['message']
                && !($cfg->isVoiceMsgEnabled()
                    && $cfg->isVoiceMsgAllowedForClients()
                    && !empty($vars['files'])))
...
class ResponseThreadEntry extends ThreadEntry {
    ...
    static function add($vars, &$errors=array()) {
        ...
        elseif (!$vars['response']
                && !($cfg->isVoiceMsgEnabled() && $cfg->isVoiceMsgAllowedForAgents()
                    && is_array($vars['files']) && count($vars['files'])))
