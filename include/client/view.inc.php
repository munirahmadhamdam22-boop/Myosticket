...
function integrateMessageComposer($form, $voiceRecorder) {
...
$(function() {
    var $filedrop = $('#reply').find('.filedrop');
    if ($filedrop.length) {
        var dropbox = $filedrop.find('.dropzone').data('dropbox');
        var uploadUrl = dropbox ? dropbox.options.url : 'ajax.php/form/upload/ticket/attach';
        var fieldName = dropbox ? dropbox.options.name : '';

         $('#voice-recorder-container').voicerecorder({
             uploadUrl: uploadUrl,
             formId: 'ticket',
         ...
         integrateMessageComposer($('#reply'), $('#voice-recorder-container'));
     }

     $('#reply').on('submit.voicerecorder', function(e) {
