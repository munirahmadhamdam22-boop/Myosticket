...
class ThreadEntryField extends FormField {
    ...
    // Allow voice-only messages: skip required validation if a
    // voice/file attachment has been uploaded (stored in session).
    if ($cfg
        && method_exists($cfg, 'isVoiceMsgEnabled')
        && $cfg->isVoiceMsgEnabled()
        && !empty($_SESSION[':uploadedFiles'])) {
    ...
    function render($options=array()) {
        ...
        ?><div id="<?php echo $id;
            ...
            <?php echo sprintf(
                ...
                '<a href="#" class="manual"><i class="icon-upload"></i>', '</a>'); ?>
