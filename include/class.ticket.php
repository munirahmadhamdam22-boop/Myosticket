...
implements RestrictedAccess, Threadable, Searchable {
   ...
   /*============== Static functions. Use Ticket::function(params); =============nolint*/
    ...
    static function create($vars, &$errors, $origin, $autorespond=true,
        ...
        // Allow voice-only tickets: message is required only when
        // there are no attachments
        if ($cfg->isVoiceMsgEnabled() && $cfg->isVoiceMsgAllowedForClients()
                && is_array($vars['files']) && count($vars['files'])) {
            $mfield = $form->getField('message');
            if ($mfield)
                $mfield->set('required', false);
        }
