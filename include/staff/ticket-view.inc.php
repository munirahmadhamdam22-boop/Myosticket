...
<?php
    foreach ($displayed as $a) {
        ...
        $isEmpty = !Format::striptags($v);
        $class = $isEmpty ? 'class="faded"' : '';
        $clean = !$isEmpty
...
>
    ...
    echo $attrs; ?>><?php echo ThreadEntryBody::clean($_POST ? $info['response'] : $draft);
                ...
                <?php if ($cfg->isVoiceMsgEnabled() && $cfg->isVoiceMsgAllowedForAgents()) { ?>
                <div id="voice-recorder-reply-container"></div>
                <?php } ?>
                </td>
            </tr>
            <tr>
    ...
    echo $attrs; ?>><?php echo ThreadEntryBody::clean($_POST ? $info['note'] : $draft);
                ...
                <?php if ($cfg->isVoiceMsgEnabled() && $cfg->isVoiceMsgAllowedForAgents()) { ?>
                <div id="voice-recorder-note-container"></div>
                <?php } ?>
                </td>
            </tr>
            <tr><td colspan="2">&nbsp;</td></tr>
...
<?php if ($cfg->isVoiceMsgEnabled() && $cfg->isVoiceMsgAllowedForAgents()) { ?>
function initStaffVoiceRecorder(containerId, formSelector) {
    var $form = $(formSelector);
    if (!$form.length) return;
    var $filedrop = $form.find('.filedrop');
    if ($filedrop.length) {
        var dropbox = $filedrop.find('.dropzone').data('dropbox');
        var uploadUrl = dropbox ? dropbox.options.url : 'ajax.php/form/upload/attach';
        var fieldName = dropbox ? dropbox.options.name : '';

        $('#' + containerId).voicerecorder({
            uploadUrl: uploadUrl,
            formId: 'ticket',
            uploadFieldId: 'attach',
        ...
        integrateStaffMessageComposer($form, $('#' + containerId));
    }
}

function integrateStaffMessageComposer($form, $voiceRecorder) {
    var $message = $form.find('#response, #internal_note').first();
    var $filedrop = $form.find('.filedrop').first();
    if (!$message.length || !$filedrop.length || !$voiceRecorder.length) return;

    var $anchor = $message.next('.-redactor-container');
    ...
    if (!$actions.find('.message-composer-send').length)
        $actions.append('<button type="submit" class="message-composer-send" title="<?php echo __('Send'); ?>" aria-label="<?php echo __('Send'); ?>"><i class="icon-plane"></i></button>');
    $filedrop.find('.dropzone').attr('title', '<?php echo __('Attach files'); ?>');
}

$(function() {
    initStaffVoiceRecorder('voice-recorder-reply-container', '#reply');
    initStaffVoiceRecorder('voice-recorder-note-container', '#note');
});
<?php } ?>
</script>
